...
public class SetOpt {
	...
	private static Field getField(Object obj, String name) {
		...
		if (res == null) {
			...
			System.err.printf("No such option: '--%s' (run --help for list of options)%n", name);
			System.exit(1);
		} else if (list.size() > 1) {
			...
			System.err.printf("Ambiguous option: '--%s' (%s) (run --help for list of options)%n", name, str);
			System.exit(1);
		}
		return res;
	}
	...
	private static Field getAbbrField(Object obj, char abbr) {
		...
		System.err.printf("No such option: '-%c' (run --help for more details)%n" + abbr);
		System.exit(1);
		return null;
	}
				...
				if (s.equals("--")) {
					...
					if (f == null) {
                        if (name.equals("help")) {
                            ...
                            System.err.printf("Unknown option '%s' (run --help for list of options)%n", name);
                            System.exit(1);
                        }
					} else if (f.getType().equals(boolean.class)) {
						if (val != null) {
							System.err.printf("The option '--%s' cannot not take arguments (run --help for more details)%n", name);
							System.exit(1);
						}
						...
						if (val == null) {
							if (i + 1 >= args.length) {
								System.err.printf("The option '--%s' requires an argument (run --help for more details)%n", name);
								System.exit(1);
							}
							val = args[++i];
						}
						try {
							...
							System.err.printf("Illegal argument for the option '--%s': %s (run --help for more details)%n", name, val);
							System.exit(1);
						}
					}
					used.add(f);
					...
					if (f == null) {
                        System.err.printf("Unknown option '%s' (run --help for list of options)%n", abbr);
						System.exit(1);
					} else if (f.getType().equals(boolean.class)) {
						if (!val.isEmpty()) {
							System.err.printf("The option '-%c' cannot not take arguments (run --help for more details)%n", abbr);
							System.exit(1);
						}
						f.set(obj, !(boolean)f.get(obj));
					} else {
						if (val.isEmpty()) {
							if (i + 1 >= args.length) {
								System.err.printf("The option '-%c' requires an argument (run --help for more details)%n", abbr);
								System.exit(1);
							}
							val = args[++i];
						}
						try {
							...
							System.err.printf("Illegal argument for the option '-%c': %s (run --help for more details)%n", abbr, val);
							System.exit(1);
						}
					}
					used.add(f);
				} else {
					System.err.printf("No such option: '-' (run --help for list of options)%n");
					System.exit(1);
				}
			}
	...
	public static void showUsage(Object obj) {
		try {
            System.err.print("Usage:\njava -cp bin Main [options] file_name\n\n-----\n");
